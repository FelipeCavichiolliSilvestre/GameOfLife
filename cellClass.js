class Cell{
	...
	*/
	setNeighbours(grid){
		...
		let g = Math.floor(this.x / this.width);
		...
		if (i != 0){
			this.neighbours.push(grid[i - 1][g]); // Superior
			if (g != 0){
				this.neighbours.push(grid[i - 1][g - 1]); // Superior-esquerda
			}
			if (g < max_collum){
				this.neighbours.push(grid[i - 1][g + 1]); // Superior-direita
			}
		}

		if (i < max_row){
			this.neighbours.push(grid[i + 1][g]); // Inferior
			if (g != 0){
				this.neighbours.push(grid[i + 1][g - 1]); // Inferior Esquerda
			}
			if (g < max_collum){
				this.neighbours.push(grid[i + 1][g + 1]); // Inferior Direita
			}
		}

		if (g < max_collum){
			this.neighbours.push(grid[i][g + 1]); // Direita
		}

		if (g != 0){
			this.neighbours.push(grid[i][g - 1]); // Esquerda
		}
	}
